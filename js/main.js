;(function(){
    ...
    function writeDataDb(request) {
        ...
        writeToStorage(json);
        tableOfIssues = new Table(tableTemplate, getStorageData("tableContent"));
        customers = new Dropdown(dropdownCustomers, getStorageData("customers"), "customer"); 
        taskTypes = new Dropdown(dropdownTaskTypes, getStorageData("taskTypes"), "type");
        submitForm = new Form(rightForm, rightFormSubmitButton, tableOfIssues);
    }
    ...
    function writeToStorage(data) {
        for (var prop in data) {
            if (!localStorage.getItem(prop)) {
                if (prop != "tableContent") data[prop].sort();
                setStorageData(prop, data[prop]);
            }
        }
    }
    ...
    function showPopup(event) {
        ...
        confirmDelete.onclick = deleteProjectConfirm(event.target);
    }
    ...
    function deleteProjectConfirm(target) {
        return function(event) {
            event.stopPropagation;
            tableOfIssues.deleteProject(target);
            closePopup(); 
        }
    }
